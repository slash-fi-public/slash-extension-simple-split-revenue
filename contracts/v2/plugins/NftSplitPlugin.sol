...
contract NftSplitPlugin is OwnableUpgradeable {
    ...
    function updateNftInfoList(
        ...
        if (_msgSender() != _operator) revert Unpermitted();

        if (chainIds_.length != tokenIds_.length) revert SplitCountMismatch();
        if (chainIds_.length != nftAddresses_.length)
            revert SplitCountMismatch();
        if (chainIds_.length != splitRates_.length) revert SplitCountMismatch();
        ...
        for (uint256 i = 0; i < splitCount; i++) {
            if (nftAddresses_[i] == address(0)) revert InvalidZeroAddress();
            _splitNfts.push(
                NftInfo({
                    chainId: chainIds_[i],
                    tokenId: tokenIds_[i],
    ...
    function updateReceiptList(address[] memory receipts_) external {
        if (_msgSender() != viewBatchContract()) revert Unpermitted();

        if (_splitNfts.length != receipts_.length) revert SplitCountMismatch();

        uint256 splitCount = receipts_.length;

        for (uint256 i = 0; i < splitCount; i++) {
            if (receipts_[i] == address(0)) revert InvalidZeroAddress();
    ...
    function updateOperator(address operator_) external {
        if (_msgSender() != owner() && _msgSender() != _operator)
            revert Unpermitted();
    ...
    function viewBatchContract() public view returns (address) {
    ...
    /**
     * @dev receive payment from SlashPayment V2 Contract
    ...
    function receivePayment(
        ...
        uint256 amount_,
        bytes calldata, /* paymentId: PaymentId generated by the merchant when creating the payment URL */
